...
sub makeHugoGeneObjects {
    ...
    foreach my $line (split /\n/, $hugoPage) {
        ...
        my $hugo = $entries[1];
        $hugo =~ s/\s+//g;
        #The hash key will be all uppercase to allow us to implement it as case insensitive
        #But the name accessible through subroutine symbol() will be in the actual case
        my $ucHugo = uc( $hugo );
        $hugoObjects{$ucHugo} = new HugoGene;
        $hugoObjects{$ucHugo}->symbol($hugo);

        (defined $entries[0] && $entries[0] ne "" ) or confess "No ID for '$hugo'";
        # Remove 'HGNC:' prefix in ID
        if ( $entries[0] =~ /HGNC\:(\d+)/ ) {
            $hugoObjects{$ucHugo}->id($1);
        }
        else {
            confess "Unexpected format for Hugo id '$entries[0]'";
        }

        if ( defined $entries[2] && $entries[2] ne "" ) { $hugoObjects{$ucHugo}->name($entries[2]); }
        if ( defined $entries[4] && $entries[4] ne "" ) { $hugoObjects{$ucHugo}->type($entries[4]); }
        if ( defined $entries[9] && $entries[9] ne "" ) { $hugoObjects{$ucHugo}->chr($entries[9]); }
        if ( defined $entries[29] && $entries[29] ne "" ) { $hugoObjects{$ucHugo}->omim($entries[29]); }
        if ( defined $entries[31] && $entries[31] ne "" ) { $hugoObjects{$ucHugo}->uniprot($entries[31]); }
        if ( defined $entries[32] && $entries[32] ne "" ) { $hugoObjects{$ucHugo}->ensembl($entries[32]); }
        if ( defined $entries[33] && $entries[33] ne "" ) { $hugoObjects{$ucHugo}->ucsc($entries[33]); }
        if ( defined $entries[16] && $entries[16] ne "" ) { $hugoObjects{$ucHugo}->addEntrezName($entries[16]); }
        if ( defined $entries[28] && $entries[28] ne "" ) { $hugoObjects{$ucHugo}->addEntrezName($entries[28]); }
        if ( defined $entries[21] && $entries[21] ne "" ) { $hugoObjects{$ucHugo}->addRefSeqName($entries[21]); }
        if ( defined $entries[30] && $entries[30] ne "" ) { $hugoObjects{$ucHugo}->addRefSeqName($entries[30]); }

        my ( $id );
        if ( defined $entries[5] && $entries[5] ne "" ) {
            foreach $id (split /\,/, $entries[5]) {
                $id =~ s/\s+//g;
                $hugoObjects{$ucHugo}->addPreviousSymbol( $id );
            }
        }
        ...
        if ( defined $entries[7] && $entries[7] ne "" ) {
            foreach $id (split /\,/, $entries[7]) {
                $id =~ s/\s+//g;
                $hugoObjects{$ucHugo}->addAlias( $id );
            }
        }

        if ( defined $entries[6] && $entries[6] ne "" ) {
            foreach $id (split /\,/, $entries[6]) {
                $hugoObjects{$ucHugo}->addOldDescription($id);
            }
        }
        ...
        if ( defined $entries[25] && $entries[25] ne "" ) {
            foreach $id (split /\,/, $entries[25]) {
                $id =~ s/\s+//g;
                $hugoObjects{$ucHugo}->addVegaName($id);
            }
        }
    }
    return \%hugoObjects;
...
sub previousHugoSymbols {
    ...
    if ( !defined $hugoObjRef ) {
        $hugoObjRef = makeHugoGeneObjects();
    }
    ...
    foreach $hugo ( keys %{$hugoObjRef} ) {
        ...
        foreach $previous ( keys %{$previousRef} ) {
            $previous = uc( $previous );
            push @{$previousToHugo{$previous}}, $$hugoObjRef{$hugo}->symbol();
...
sub hugoAlias {
    ...
    foreach $hugo ( keys %{$hugoObjRef} ) {
        ...
        foreach $alias ( keys %{$aliasRef} ) {
            $alias = uc( $alias );
            push @{$aliasToHugo{$alias}}, $$hugoObjRef{$hugo}->symbol();
...
sub isUpdatedHugoGeneRecord {
    ...
    my ( $entrezMismatch, $ensemblMismatch, $omimMismatch, $vegaMismatch, $refSeqMismatch,
         $uniprotMismatch, $ucscMismatch, $mismatchTotal, $entrezMatch, $ensemblMatch, $omimMatch,
         $vegaMatch, $refSeqMatch, $uniprotMatch, $ucscMatch, $matchTotal, $invalidSymbol,
         $hugoIdMatch, $hugoIdMismatch );

    $invalidSymbol = $$invalidRef->symbol();

    $uniprotMismatch = $ensemblMismatch = $omimMismatch = $ucscMismatch = $vegaMismatch = 0;
    $refSeqMismatch = $entrezMismatch = $entrezMatch = $ensemblMatch = $omimMatch = 0;
    $vegaMatch = $refSeqMatch = $uniprotMatch = $ucscMatch = $hugoIdMatch = $hugoIdMismatch = 0;
...
sub ensemblToHugoId {
    # Return ref to hash of hash with key = Ensembl ID, Hugo ID, value = 1
    ...
    $hugoGeneRef = makeHugoGeneObjects();
...
sub loadHugoPage {
    #If a local version of the page exists, use that instead of querying the website
    my $hugoPage = '';
    if( -e $HugoLocal ) {
        my $fh = IO::File->new( $HugoLocal );
        my @hugoLines = $fh->getlines;
        $fh->close;
        $hugoPage = join( '', @hugoLines );
    }
    else {
        $hugoPage = get( $HugoRemote );
        my $fh = IO::File->new( $HugoLocal, ">" );
        $fh->print( $hugoPage );
        $fh->close;
    }
